...
#define mode_select_icon_x2 0   //模式选择图标img2x坐标
...
{
	//grf_printf("第一次进入显示的界面 languageBuf[ytl3_switch_language] == %s\n",languageBuf[ytl3_switch_language]);
	i = 1;
	grf_ctrl_move_forground(img0);
	grf_ctrl_move_background(img2);
	...
	{
			grf_img_set_src(img0, select_mode_array1[8]);
			system_setting_value_array[0] = "#switch_language";
			grf_label_set_txt(img0_label, "#language");
			grf_ctrl_set_pos(img0_label,90,265);
	}
	else {
		if (ytl0_roller_brush_lighting) {
			grf_img_set_src(img0, select_mode_array1[1]);
			system_setting_value_array[0] = "#roller_brush_lighting";
			grf_label_set_txt(img0_label, "#on");
			grf_ctrl_set_pos(img0_label,90,242);
	...
	set_img_label_pivot();
	grf_label_set_txt(system_setting_label_name_ID1, system_setting_value_array[i-1]);  //1滚刷照明,2清洁方式,3语音音量,4切换语言
}
...
static void switch_highlight()
{
	if (i == 1) {
		...
		if (*languageBuf[ytl3_switch_language] == *"tu")  //如果是土耳其语就把滚刷照明改成语言图标
		{
			grf_img_set_src(img0, select_mode_array2[8]);
		}
		else {
			if (ytl0_roller_brush_lighting) {
...
static void on_off_Voice_broadcast()
{
	...
	if (ytl2_voice_image_val == 0)
	{
		switch_language_pack("17_00_close_voice");  //关闭语音
	}
	else if (ytl2_voice_image_val == 1)
	{
		switch_language_pack("17_01_enable_voice");  //开启语音
	}
}

static u8 ytl_confirmation_v = 0;  //确认键设置按键太快的丢掉
u8 mute_key_voice = 2;  //判断在哪个音量按了静音键关闭了语音,在按静音键开启语音回到之前关闭语音的音量
//按键任务
void key_task17_cb()
{
	//丢掉太快的选择键
	control_key_failure();
	//丢掉太快的确认键
	if (ytl_confirmation_v <= 5) {
		ytl_confirmation = GRF_FALSE;
		ytl_confirmation_v++;
		//grf_printf("ytl_confirmation_v == %d\n",ytl_confirmation_v);
	}
	else if (ytl_confirmation_v > 5 && ytl_confirmation)
	{
	...
	if (ytl_right)
	{
		...
		for(u8 var=0;var<3;var++)
		{
			...
			if(grf_ctrl_get_x(obj)==mode_select_icon_x0)
				yk_animation_set(obj, switch_speed,0,0,mode_select_icon_x0,mode_select_icon_x2,anim_img_pos,scale,scaling*scale,anim_img_zoom);
			...
			if(grf_ctrl_get_x(img0)==mode_select_icon_x0)
				yk_animation_set(img0_label, switch_speed, 0,0,0,0,NULL, scale, scale*scaling, anim_label_zoom);
			else if(grf_ctrl_get_x(img0)==mode_select_icon_x1)
				yk_animation_set(img0_label, switch_speed, 0,0,0,0,NULL, scale*scaling, scale, anim_label_zoom);

			if(grf_ctrl_get_x(img1)==mode_select_icon_x0)
				yk_animation_set(img1_label, switch_speed, 0,0,0,0,NULL, scale, scale*scaling, anim_label_zoom);
			else if(grf_ctrl_get_x(img1)==mode_select_icon_x1)
				yk_animation_set(img1_label, switch_speed, 0,0,0,0,NULL, scale*scaling, scale, anim_label_zoom);

			if(grf_ctrl_get_x(img2)==mode_select_icon_x0)
				yk_animation_set(img2_label, switch_speed, 0,0,0,0,NULL, scale, scale*scaling, anim_label_zoom);
			else if(grf_ctrl_get_x(img2)==mode_select_icon_x1)
				yk_animation_set(img2_label, switch_speed, 0,0,0,0,NULL, scale*scaling, scale, anim_label_zoom);
		...
		//grf_printf("右键值i == %d\n",i);
		switch_highlight();
		grf_label_set_txt(system_setting_label_name_ID1, system_setting_value_array[i-1]);  //1滚刷照明,2语音音量,3清洁方式
	}
	else if (ytl_back)
	{
		ytl_back = GRF_FALSE;
	...
	{
		//grf_printf("ytl_confirmation i == %d*****************************************************************************\n",i);
		ytl_confirmation = GRF_FALSE;
		if (i == 1)
		{
	...
	else if (ytl_mute)
	{
		...
		if (i == 3)
		{
			...
			if (ytl2_voice_image_val == 0)
			{
				...
				else if (mute_key_voice == 2 || mute_key_voice == 0)
				{
					grf_img_set_src(img2, select_mode_array1[6]);  //音量中
					grf_label_set_txt(img2_label, "#medium");
					grf_label_set_txt_color(img2_label, GRF_COLOR_GET(0xf9,0x8a,0x13));
		...
		{
			volume_size();
			//按静音键语音播报,如果是在关闭语言按了静音键，就都播报开启语音
			if (ytl2_voice_image_val == 0)
			{
				switch_language_pack("17_00_close_voice");  //关闭语音
			}
